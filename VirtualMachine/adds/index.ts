...
const deploy_spec = [
    {
        ...
        category: [
            {
                cluster: "Cluster",
                tags: ["norther", "adds"], // [Project, Group]
                datastore: "ds_node01",
                template: "template_Windows2019",
                network: "VM Network",
                domain: "home.local",
                gateway: "192.168.0.1",
                ipv4netmask: 24,
                ...
                timezone: 210,
                virtualmachine: [
                    { name: "adds00", ip: "192.168.0.149", cpus: 2, memory: 8192, disk: 100 },
                ]
            },
        ]
    }
            ...
            let virtualmachine = new vsphere.VirtualMachine(deploy_spec[i].category[category_index].virtualmachine[vm_index].name + ".node." + deploy_spec[i].category[category_index].domain, {
                ...
                clone: {
                    ...
                    customize: {
                        ...
                        windowsOptions: {
                            computerName: deploy_spec[i].category[category_index].virtualmachine[vm_index].name,
                            adminPassword: deploy_spec[i].category[category_index].adminpassword,
                            timeZone: deploy_spec[i].category[category_index].timezone
                        },
                        networkInterfaces: [
                            {
